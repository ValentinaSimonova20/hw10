...
public class RedBlackTree<K extends Comparable<K>> {
    ...
    public void setWinnerBy(K winner, K[] allParticipants) {
        for(K participant: allParticipants) {
            if(participant != winner) {
                Node<K> loserNode = get(participant);
                if(loserNode != null && loserNode.winnerBy == null) {
                    loserNode.winnerBy = winner;
                }
            }
        }
    }
    ...
    public K getWinnerBy(K key) {
        Node<K> node = get(key);
        return node!= null ? node.winnerBy : null;
    }
